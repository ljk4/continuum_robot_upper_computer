...
1. **图像伺服搜索模式（Search）需眼在手上**：当前摄像头为固定安装（眼在手外），Search 模式下摄像头应装在机器人末端。现阶段该模式仅通过纯像素偏差模拟验证了控制逻辑，末端随动的实际效果待机械改装后测试。

2. **所有实现仅经仿真验证**：当前测试均基于 `fake_stm32.py` 虚拟下位机 + MuJoCo 3D 渲染，未接入真实 STM32 和舵机。实物联调时可能出现通信延迟、舵机死区、机械间隙等仿真中不存在的问题。

3. **参数需实物调试时逐步调整**：`config.py` 中的 IK 收敛容差、弯曲角上限、绳长限幅、轨迹频率、搜索增益（`_Kp`/`_Ka`）等参数均为仿真中调定的初始值，实物联调时需根据实际硬件响应逐步修正。
